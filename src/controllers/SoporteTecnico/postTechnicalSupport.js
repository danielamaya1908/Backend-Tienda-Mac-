...
const postTechnicalSupport = async (req, res) => {
  try {
    const {
      marca, modelo, serial, DocumentNumber, garantia, enciende, arranca, parlantes, teclado,
    ...
    if (!marca || !modelo || !serial || !DocumentNumber) {
      return res.status(400).json({ error: 'Marca, modelo, serial y DocumentNumber son campos requeridos' });
    }

    // Verificar si el documento existe
    const user = await User.findOne({ where: { documentNumber: DocumentNumber } });
    if (!user) {
    ...
    console.error('Error al crear soporte técnico:', error.message, error.stack);
    return res.status(500).json({ error: 'Error interno del servidor' });
  }
};

module.exports = { postTechnicalSupport, uploadMiddleware };
